...
console.log('🚨 [MODULE LOADING] orderTagService.js is being loaded');
...
class OrderTagService {
  ...
  async fetchGroups(productId, portion) {
    ...
    debug('fetchGroups called', {
      ...
      let terminalName = "SERVIDOR"; // Default fallback
      ...
      if (user?.name) {
        ...
        if (!terminalId) {
          ...
          console.log('⚠️ [ORDER TAG SERVICE] No terminal registered');
        } else {
          debug('✅ Terminal confirmed for getOrderTagGroups:', terminalId);
          console.log('✅ [ORDER TAG SERVICE] Terminal confirmed:', terminalId);

          // Try to get terminal name from user or default to SERVIDOR
          terminalName = user.terminalName || "SERVIDOR";
          console.log('🏷️ [ORDER TAG SERVICE] Using terminal name:', terminalName);
        }
      }
  ...
  async getGroups(productId, portion) {
    ...
    console.log('🚨🚨🚨 [ORDER TAG SERVICE] *** getGroups FUNCTION ENTRY - SQL VERSION *** 🚨🚨🚨');
    console.log('📞 [ORDER TAG SERVICE] getGroups parameters:', { productId, portion });

    // First try SQL endpoint (bypasses GraphQL cache issues)
    try {
      const sqlResult = await this.fetchGroupsSQL(productId, portion);
      if (sqlResult && sqlResult.length > 0) {
        console.log('✅ [ORDER TAG SERVICE] SQL endpoint successful, returning:', sqlResult);
        return sqlResult;
      }
    ...
    // Fallback to original GraphQL cache method
  ...
  async fetchGroupsSQL(productId, portion) {
    console.log('🌐 [ORDER TAG SERVICE] fetchGroupsSQL called with:', {
      productId: productId,
      portion: portion
    });

    try {
      const baseUrl = process.env.REACT_APP_SERVER_URL || 'http://localhost:4005';
      const endpoint = `${baseUrl}/internal-api/order-tags/${productId}/${encodeURIComponent(portion || 'Normal')}`;

      console.log('📡 [ORDER TAG SERVICE] Fetching from SQL endpoint:', endpoint);
      ...
      console.log('📥 [ORDER TAG SERVICE] SQL endpoint response:', data);
      ...
      for (const group of data.tagGroups) {
        for (const tag of (group.tags || [])) {
          const tagObj = {
            id: `${group.name}:${tag.name}`,
            name: tag.name,
            group: group.name,
            price: tag.price || 0,
            maxSelection: group.maxSelection,
            requiredSelection: group.minSelection
          };
          flat.push(tagObj);
      ...
      console.error('❌ [ORDER TAG SERVICE] fetchGroupsSQL error:', error);
      throw error;
    }
  }
