...
console.log('🔧 [SERVER INIT] Middleware de autenticación configurado para /internal-api');
console.log('🔧 [SERVER INIT] Registrando endpoints...');
...
app.get('/internal-api/sql-config', async (req, res) => {
    try {
        ...
        const db = require('./lib/db');
        const config = sqlConfig.getSanitizedConfig();
        let connected = false;
        try {
            await db.query('SELECT TOP 1 1 AS ok');
            connected = true;
        } catch (err) {
            debug('SQL config health check failed', err && err.message);
        }
        res.json({ ok: true, config, connected });
    } catch (err) {
        res.status(500).json({ ok: false, error: err?.message || 'Failed to load SQL configuration' });
    }
});

app.post('/internal-api/sql-config/test', async (req, res) => {
    try {
        const sqlConfig = require('./lib/sqlConfig');
        await sqlConfig.testConnection(req.body || {});
        res.json({ ok: true, message: 'Conexión exitosa' });
    } catch (err) {
        res.status(400).json({ ok: false, error: err?.message || 'No se pudo conectar con SQL Server' });
    }
});

app.post('/internal-api/sql-config', async (req, res) => {
    try {
        const sqlConfig = require('./lib/sqlConfig');
        const db = require('./lib/db');
        sqlConfig.saveConfig(req.body || {});
        await db.resetPool();
        res.json({ ok: true, message: 'Configuración aplicada' });
    } catch (err) {
        res.status(400).json({ ok: false, error: err?.message || 'No se pudo guardar la configuración' });
    }
});

app.post('/internal-api/sql-config/reload', async (req, res) => {
    try {
        const db = require('./lib/db');
        await db.resetPool();
        res.json({ ok: true, message: 'Conexión SQL reiniciada' });
    } catch (err) {
        res.status(500).json({ ok: false, error: err?.message || 'No se pudo reiniciar la conexión' });
    }
});
 ...
 *         description: Database error
 */
console.log('🔧 [SERVER INIT] Registrando endpoint /internal-api/order-tags/:productId/:portion');
app.get('/internal-api/order-tags/:productId/:portion', async (req, res) => {
    const requestId = req.requestId || 'unknown';
    console.log(`🏷️ [DEBUG] Order tags endpoint called with productId: ${req.params.productId}, portion: ${req.params.portion}`);
    ...
    try {
        ...
        console.log(`🏷️ [DEBUG] Parsed parameters: productId=${productId}, portion=${portion}`);

        if (!productId) {
            debug(`⚠️ [${requestId}] Invalid productId: ${req.params.productId}`);
            console.log(`❌ [DEBUG] Invalid productId: ${req.params.productId}`);
            return res.status(400).json({ error: 'Invalid product ID' });
        }

        debug(`🏷️ [${requestId}] Fetching order tags for product ${productId}, portion: ${portion}`);
        console.log(`🔍 [DEBUG] About to require db module...`);
        const db = require('./lib/db');
        console.log(`✅ [DEBUG] DB module loaded successfully`);
        ...
        console.log(`🔍 [DEBUG] Raw query returned ${tagGroups.length} tag records`);
        console.log(`🔍 [DEBUG] Sample records:`, tagGroups.slice(0, 3));
        ...
        console.log(`✅ [DEBUG] Final response: ${finalTagGroups.length} groups, ${totalTags} total tags`);
        console.log(`✅ [DEBUG] Final tag groups:`, finalTagGroups);
        ...
        console.error(`❌ [DEBUG] Error in order tags endpoint:`, err);
